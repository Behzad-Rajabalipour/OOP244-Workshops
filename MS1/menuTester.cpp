...
int main() {
   ...
   if (m2) {
      ...
      cout << "The >" << m2 << "< menu is empty" << endl;           // Menu.h => #ref3
   }
   m1 << "Omelet" << "Tuna Sandwich" << "California Roll";          // Menu.h => #ref2
   m2 << "Order more";
   if (m1) {
      cout << "The " << m1 << " is not empty and has " << (unsigned int)(m1) << " menu items." << endl;         // Menu.cpp => #ref1
   }
   do {
      s = m1.run();
      if (s) {                                      // agar s = 0 bashe yani false
         selections[s - 1]++;
